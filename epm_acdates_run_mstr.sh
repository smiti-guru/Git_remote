...
sed  -f $epm_nonsec_fil -f $epm_sec_fil \
    -e "s,&pst_per,${pst_per},g" \
    -e "s,&rpt_ver_cd,${rpt_ver_cd},g" \
    -e "s,__LOGNAME__,${LOGNAME},g" \
    -e "s,__TWS_SCHED__,$TWS_SCHED,g" \
$EPM_PGM_DIR/`basename $0 .sh`.btq > $EPM_TEMPFILS_DIR/`basename $0 .sh`_btq.tmp
bteq < $EPM_TEMPFILS_DIR/`basename $0 .sh`_btq.tmp

rm $EPM_TEMPFILS_DIR/`basename $0 .sh`_btq.tmp
